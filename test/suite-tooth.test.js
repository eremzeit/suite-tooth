...
const { suite, executeTestSuite, executeRootSuite, skip, only } = require('../src/suite-tooth')
...
it('beforeEach hooks should execute', async (t) => {
  return executeSuiteAndVerify({
    'FOO': {
      'BAR': {
...
it('Each of the types of hooks', async () => {
  executeSuiteAndVerify({
    'FOO': {
...
it('if there are no test cases, the lifecycle hooks dont run', async () => {
  return executeSuiteAndVerify({
      __beforeAll: [(ctx) => {
        return {
          count: 0,
...
describe('flagging tests with SKIP', () => {
  it('tests marked with skip dont run', () => {
    return executeSuiteAndVerify({
  ...
  it('tests prefixed with SKIP dont run', () => {
    return executeSuiteAndVerify({
...
describe('flagging tests with ONLY', () => {
  it('if a test function is marked as only, the others are skipped', async () => {
    return executeSuiteAndVerify({
  ...
  it('prefixing the test key with the ONLY string flags that test as ONLY', async () => {
    return executeSuiteAndVerify({
  ...
  it('if no descendant tests are marked as only then all descendant tests run', async () => {
    return executeSuiteAndVerify({
      'foo': only({
        'foo-1': () => {
          return {
  ...
  it('tests flagged as "only" within an array', async () => {
    return executeSuiteAndVerify({
      'foo': [
        only(() => {
          return {
...
it.skip('when there is an error thrown in a testing function, the tap output shows a failure', async () => {
  return executeSuiteAndVerify({
    'FOO': () => {
    ...
    expectTapIsEqual(tapOutput.replace(/(?<=origin[:] [|][-])[\s\S]*?\.\.\./mg, ''), stripIndent`
      TAP version 13
      # Subtest: FOO
          not ok 1 - foo error
            ---
...
it('beforeEach hooks should execute', async () => {
  return executeSuiteAndVerify({
    'FOO': {
